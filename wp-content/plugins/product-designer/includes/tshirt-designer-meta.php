...
function product_designer_inner_custom_box( $post ) {
	...
	//echo '<pre>'.var_export($templates, true).'</pre>';
...
.tshirt-hint{
    ...
    <div class="option-box">
    	<p class="option-title"><?php echo __('Item sides', product_designer_textdomain); ?></p>
    </div>
         ...
         jQuery(document).ready(function($)
            {
				...
				//$( ".items-container" ).disableSelection();
					...
					$(document).on('click','.side .remove, .template .remove',function(){
...
<?php
			if(!empty($templates)){
				...
				foreach($templates as $side_id=>$template_group){
					...
					//$default = $template_group['default'];
