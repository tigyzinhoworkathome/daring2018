jQuery(document).ready(function($)
	...
	$(document).on('change','.sticker-cat',function(){
		
		
			var per_page = $(".sticker-load-more").attr('per_page');			

			$(".sticker-load-more").attr('offset', per_page);		
			$('.sticker-load-more').css('background','rgb(255, 185, 33)');
			$('.sticker-load-more').prop('disabled', false);
			$('.sticker-load-more').css('cursor', 'default');

			$(".sticker-list").html('');
			$('.sticker-cat-loading').fadeIn();
			
			var sticker_terms = $(this).val();			
			var offset = 0;			
			...
			data: {"action": "tshirt_designer_get_sticker_list_by_cat_ajax","offset":offset,"sticker_terms":sticker_terms},
	...
	$(document).on('change','.product-cat',function(){
		
		
			var per_page = $(".product-load-more").attr('per_page');			

			$(".product-load-more").attr('offset', per_page);		
			$('.product-load-more').css('background','rgb(255, 185, 33)');
			$('.product-load-more').prop('disabled', false);
			$('.product-load-more').css('cursor', 'default');

			$(".product-list").html('');
			$('.product-cat-loading').fadeIn();
			
			var product_terms = $(this).val();			
			var offset = 0;			
			...
			type: 'POST',
			url: tshirt_designer_ajax.tshirt_designer_ajaxurl,
			data: {"action": "tshirt_designer_product_list_by_cat_ajax","offset":offset,"product_terms":product_terms},
			success: function(data)
					{
						
						$(".product-list").append(data);
						$('.product-cat-loading').fadeOut();
	...
	$(document).on('click','.sticker-load-more',function(){
		...
		type: 'POST',
		url: tshirt_designer_ajax.tshirt_designer_ajaxurl,
		data: {"action": "tshirt_designer_get_sticker_list_ajax","offset":offset,"sticker_terms":sticker_terms},
		success: function(data)
				{
	...
	$(document).on('click','.product-load-more',function(){
		...
		var product_terms = $('.product-cat').val();
		var offset = parseInt($(this).attr("offset"));
		var per_page = parseInt($(this).attr("per_page"));		
		...
		type: 'POST',
		url: tshirt_designer_ajax.tshirt_designer_ajaxurl,
		data: {"action": "tshirt_designer_get_product_list_ajax","offset":offset,"product_terms":product_terms},
		success: function(data)
				{
	...
	$(document).on('click', '.preview', function(){
		...
		is_img_sticker = $("#canvas").find(".sticker-img");
		console.log(is_img_sticker.length);
		if(is_img_sticker.length==0){
			
			alert('Please add at least one scticker.');
	...
	$(document).on('click', '.inserttext', function(){
		...
		$(document).on('click', '.td-navs .td-nav', function(){
				//alert('Hello');
				...
				$(".td-nav-boxs .td-nav-box").css("display","none");
